...
public class OpenIdManager {
     ...
     * @deprecated Using getAuthentication(Map request, byte[] key, String alias) instead.
     */
    public Authentication getAuthentication(Map<String, String> request, byte[] key) {
     ...
     */
    public Authentication getAuthentication(Map<String, String> request, byte[] key, String alias) {
        // verify:
        String identity = request.get("openid.identity");
        if (identity==null)
            throw new OpenIdException("Missing 'openid.identity'.");
        if (request.get("openid.invalidate_handle")!=null)
            throw new OpenIdException("Invalidate handle.");
        String sig = request.get("openid.sig");
        if (sig==null)
            throw new OpenIdException("Missing 'openid.sig'.");
        String signed = request.get("openid.signed");
        if (signed==null)
            throw new OpenIdException("Missing 'openid.signed'.");
        if (!returnTo.equals(request.get("openid.return_to")))
        ...
        for (String param : params) {
            sb.append(param)
              .append(':');
            String value = request.get("openid." + param);
            if (value!=null)
                sb.append(value);
            sb.append('\n');
        }
        ...
        auth.setEmail(request.get("openid." + alias + ".value.email"));
        auth.setLanguage(request.get("openid." + alias + ".value.language"));
        auth.setGender(request.get("openid." + alias + ".value.gender"));
    ...
    String getLastname(Map<String, String> request, String axa) {
      String name = request.get("openid." + axa + ".value.lastname");
      // If lastname is not supported try to get it from the fullname
      if (name == null) {
        name = request.get("openid." + axa + ".value.fullname");
        if (name != null) {
    ...
    String getFirstname(Map<String, String> request, String axa) {
      String name = request.get("openid." + axa + ".value.firstname");
    ...
    String getFullname(Map<String, String> request, String axa) {
      // If fullname is not supported then get combined first and last name
      String fname = request.get("openid."+axa+".value.fullname");
      if (fname == null) {
        fname = request.get("openid."+axa+".value.firstname");
        if (fname != null) {
          fname += " ";
        }
        fname += request.get("openid."+axa+".value.lastname");
      }
      return fname;
    }
